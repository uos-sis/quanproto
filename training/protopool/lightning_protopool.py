...
from quanproto.models.config_parser import create_protopool
from quanproto.models.protopool.optimizer import make_optimizer
from quanproto.models.protopool.protopool import GumbleScaleCallback, compute_loss
...
def train(
    ...
    parameter_config = deepcopy(
        parameter.protopool_params[experiment_config["dataset"]]
    )
    ...
    model.compile(
        coefs={
            ...
            "orth": config["coefs.orth"],
            "l1": config["coefs.l1"],
            "tau_start": config["coefs.tau_start"],
            "tau_end": config["coefs.tau_end"],
            "decreasing_interval": config["coefs.decreasing_interval"],
        },
    ...
    # endregion Optimizer and Scheduler ------------------------------------------------------------
    ...
    gumble_callback = GumbleScaleCallback()
    try:
        if tune_config == None:
            acc = quan_training.train_lightning(
                ...
                callbacks=[gumble_callback, save_callback],
            ...
            acc = quan_training.tune_lightning(
                ...
                train_loader=train_loader,
                validation_loader=validation_loader,
                callbacks=[gumble_callback, save_callback, prune_callback],
            )
    # catch all errors
    ...
    # we are done here, the last part is only for saving the model to wandb if it is used
    logger.finish()
    return acc
...
def objective(trial: optuna.Trial) -> float:
    tune_config = {
        "warmup_epochs": trial.suggest_int("warmup_epochs", 10, 30, step=5),
        ...
        "joint_optimizer_lr.add_on_layers": trial.suggest_float(
            "joint_optimizer_lr.add_on_layers",
            1e-5,
            3e-3,
            log=True,
        ),
        ...
        "fine_tune_optimizer_lr": trial.suggest_float(
            "fine_tune_optimizer_lr",
            1e-5,
            3e-3,
            log=True,
        ),
        "coefs.decreasing_interval": trial.suggest_int(
            "coefs.decreasing_interval", 10, 60, step=10
        ),
    }
...
if __name__ == "__main__":
    if not args.tune:
        ...
        study = optuna.create_study(
            direction="maximize",
            pruner=optuna.pruners.MedianPruner(n_warmup_steps=40, interval_steps=10),
        )
