"""
The file contains the ProtoPNet model. 
...
from quanproto.models.interfaces.prototype_model_interfaces import PrototypeModelInterface
...
from quanproto.models.helper import convert_to_multilabelmargin_input


class ProtoPNet(nn.Module, PrototypeModelInterface):
    def __init__(
        self,
        backbone: nn.Module,
        num_labels: int = 200,
        ...
        self.prototype_shape: tuple[int, int, int, int] = (self._num_prototypes,) + prototype_shape
        ...
        self.prototype_vectors = nn.Parameter(
            torch.FloatTensor(*self.prototype_shape).uniform_(0.4, 1.0), requires_grad=True
        )
    ...
    def compile(
        self,
        coefs: dict | None = {"crs_ent": 1, "clst": 0.8, "sep": 0.08, "l1": 1e-3},
    ...
    def _initialize_weights(self) -> None:
        ...
        self._last_layer_incorrect_connection(incorrect_strength=0.0)
    ...
    def forward(self, x):
        ...
        # minimum distances of each prototype
    ...
    def explain(self, x):
        ...
        similarity_scores = torch.functional.F.max_pool2d(
            similarity_maps, kernel_size=similarity_maps.size()[2:]
        ).squeeze()
    ...
    def push_forward(self, x):
        """
        The push forward function is used to push the prototypes to the feature space.

        Returns:
            feature maps (BxCxHxW)
            distance maps (BxPxHxW)
        """
        x = self.backbone(x)
...
def compute_loss(
    ...
    if pl_model.model.prototype_class_identity.device != logits.device:
        prototypes_class_identity = pl_model.model.prototype_class_identity.to(logits.device)
    else:
        prototypes_class_identity = pl_model.model.prototype_class_identity

    # make a tensor with the same size as the target tensor but with the arange values
    ...
    if pl_model.model.multi_label:
        ...
        prototypes_of_correct_class = target * prototypes_class_identity.transpose(0, 1)
        inverted_distances, _ = torch.max(
            (max_dist - min_distances.unsqueeze(1)) * prototypes_of_correct_class, dim=2
        )
        ...
        prototypes_of_wrong_class = (1 - target) * prototypes_class_identity.transpose(0, 1)

        inverted_distances, _ = torch.max(
            (max_dist - min_distances.unsqueeze(1)) * prototypes_of_wrong_class, dim=2
        )
        ...
        prototypes_of_correct_class = torch.t(prototypes_class_identity[:, target])
        inverted_distances, _ = torch.max(
    ...
    l1_mask = 1 - torch.t(prototypes_class_identity)
