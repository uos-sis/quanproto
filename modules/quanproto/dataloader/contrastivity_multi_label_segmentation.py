    ...
    def __getitem__(self, idx: int):
        ...
        for i in range(num_masks):
            ...
            if original_mask.shape[2] == 4:
                original_mask = (ski.color.rgba2rgb(original_mask) * 255).astype(
                    np.uint8
                )
            ...
            if hasattr(self, "bboxes"):
                ...
                if self.min_bbox_size > 0:
                    bboxes = expand_bboxes(bboxes, (width, height), self.min_bbox_size)[
                        0
                    ]
            ...
            uncropped_masks = torch.cat(
                (uncropped_masks, uncropped_mask.unsqueeze(0)), 0
            )

        return (masks, uncropped_masks), label, class_id
