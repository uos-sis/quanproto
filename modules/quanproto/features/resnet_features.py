"""
Implementation of ResNet features in PyTorch.
...
class ResNet_features(nn.Module):
    ...
    # TODO add an array with strides or something to change the ouput map size
    def __init__(self, block, layers, strides=[1, 2, 2, 2], zero_init_residual=False):
        ...
        self.layer1 = self._make_layer(
            block=block, planes=64, num_blocks=self.layers[0], stride=strides[0]
        )
        self.layer2 = self._make_layer(
            block=block, planes=128, num_blocks=self.layers[1], stride=strides[1]
        )
        self.layer3 = self._make_layer(
            block=block, planes=256, num_blocks=self.layers[2], stride=strides[2]
        )  # TODO: PIPNet uses one here
        self.layer4 = self._make_layer(
            block=block, planes=512, num_blocks=self.layers[3], stride=strides[3]
        )  # TODO: PIPNet uses one here
...
def resnet18_features(pretrained=False, **kwargs):
    ...
    """
    model = ResNet_features(BasicBlock, [2, 2, 2, 2], [1, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet18_ext_features(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(BasicBlock, [2, 2, 2, 2], [1, 2, 1, 1], **kwargs)
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["resnet18"], model_dir=model_dir)
        my_dict.pop("fc.weight")
        my_dict.pop("fc.bias")
...
def resnet34_features(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(BasicBlock, [3, 4, 6, 3], [1, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet34_ext_features(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(BasicBlock, [3, 4, 6, 3], [1, 2, 1, 1], **kwargs)
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["resnet34"], model_dir=model_dir)
        my_dict.pop("fc.weight")
        my_dict.pop("fc.bias")
...
def resnet50_features(pretrained=False, inat=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(Bottleneck, [3, 4, 6, 3], [1, 2, 2, 2], **kwargs)
    if pretrained:
        if inat:
...
def resnet50_ext_features(pretrained=False, inat=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(Bottleneck, [3, 4, 6, 3], [1, 2, 1, 1], **kwargs)
    if pretrained:
        if inat:
            model_dict = torch.load(model_urls["resnet50Nat"])
            new_model = copy.deepcopy(model_dict)
...
def resnet101_features(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(Bottleneck, [3, 4, 23, 3], [1, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet101_ext_features(pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(Bottleneck, [3, 4, 23, 3], [1, 2, 1, 1], **kwargs)
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["resnet101"], model_dir=model_dir)
        my_dict.pop("fc.weight")
        my_dict.pop("fc.bias")
...
def resnet152_features(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(Bottleneck, [3, 8, 36, 3], [1, 2, 2, 2], **kwargs)
    if pretrained:
...
def resnet152_ext_features(pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_features(Bottleneck, [3, 8, 36, 3], [1, 2, 1, 1], **kwargs)
    if pretrained:
        my_dict = model_zoo.load_url(model_urls["resnet152"], model_dir=model_dir)
        my_dict.pop("fc.weight")
        my_dict.pop("fc.bias")
...
if __name__ == "__main__":
    ...
    # ouput_size = model_output_size(resnet, (3, 224, 224))
    # print(f"Output size: {ouput_size}")

    r50_features = resnet18_ext_features(pretrained=True)
    summary(r50_features, (3, 224, 224), device="cpu")
