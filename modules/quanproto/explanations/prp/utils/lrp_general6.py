...
def get_lrpwrapperformodule(
    ...
    if isinstance(module, nn.ReLU):
        ...
        return oneparam_wrapper_class(
            module, autogradfunction=autogradfunction, parameter1=lrp_params["linear_eps"]
        )
        ...
        autogradfunction = lrp_layer2method[key]()
        return oneparam_wrapper_class(
            module, autogradfunction=autogradfunction, parameter1=lrp_params["pooling_eps"]
        )

    elif isinstance(module, nn.AdaptiveMaxPool2d):
        key = "nn.AdaptiveMaxPool2d"
        if key not in lrp_layer2method:
            print("found no dictionary entry in lrp_layer2method for this module name:", key)
            raise lrplookupnotfounderror(
                "found no dictionary entry in lrp_layer2method for this module name:", key
        ...
        # default adaptivemaxpool2d_wrapper_fct()
        autogradfunction = lrp_layer2method[key]()
        return zeroparam_wrapper_class(module, autogradfunction=autogradfunction)
        ...
        autogradfunction = lrp_layer2method[key]()
        return oneparam_wrapper_class(
            module, autogradfunction=autogradfunction, parameter1=lrp_params["eltwise_eps"]
        )
...
def convafterbn_returntensorbiasedconv(conv, bn):  # after visatt
    ...
    if conv.padding == 0:
        ...
        inputfornewbias2 = (
            inputfornewbias.unsqueeze(1).unsqueeze(2).repeat((1, ksize[0], ksize[1])).unsqueeze(0)
...
class conv2d_zbeta_wrapper_fct(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        input_, conv2dweight, conv2dbias, lrpignorebiastensor, lowest_, highest_, *values = (
            ctx.saved_tensors
        )
        ...
        with torch.enable_grad():
            ...
            S = safe_divide(
                grad_output[0].clone().detach(), Z.clone().detach(), eps0=1e-6, eps=1e-6
            )
            Z.backward(S)
...
class adaptiveavgpool2d_wrapper_fct(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        X = input_.clone().detach().requires_grad_(True)
        R = lrp_backward(
            _input=X, layer=layerclass, relevance_output=grad_output[0], eps0=eps, eps=eps
        )
...
class adaptivemaxpool2d_wrapper_fct(torch.autograd.Function):
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
    ...
    def backward(ctx, grad_output):
        ...
        # class instantiation
        layerclass = torch.nn.AdaptiveMaxPool2d(**paramsdict)
...
class maxpool2d_wrapper_fct(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        X = input_.clone().detach().requires_grad_(True)
        # R = lrp_backward(
        #     _input=X, layer=layerclass, relevance_output=grad_output[0], eps0=1e-12, eps=0
        # )
        ...
        Z.backward(relevance_output_data)
        R = X.grad  # use 1 as relevance
        # R = X.data * X.grad.data  # use the original maxpool output as relevance
...
class softmax_wrapper_fct(torch.autograd.Function):
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
...
class posnegconv(nn.Module):
    ...
    def __init__(self, conv, ignorebias):
        ...
        self.posconv.weight = torch.nn.Parameter(conv.weight.data.clone().clamp(min=0)).to(
            conv.weight.device
        )

        self.negconv = self._clone_module(conv)
        self.negconv.weight = torch.nn.Parameter(conv.weight.data.clone().clamp(max=0)).to(
            conv.weight.device
        )
...
class anysign_conv(nn.Module):
    ...
    def __init__(self, conv, ignorebias):
        ...
        if ignorebias == True:
            ...
            if conv.bias is not None:
                self.posconv.bias = torch.nn.Parameter(conv.bias.data.clone().clamp(min=0)).to(
                    conv.weight.device
                )
                self.negconv.bias = torch.nn.Parameter(conv.bias.data.clone().clamp(max=0)).to(
                    conv.weight.device
                )
...
class tensorbiasedconv2d_beta0_wrapper_fct(torch.autograd.Function):
    ...
    def forward(ctx, x, tensorbiasedclass, lrpignorebias):
        ...
        lrpignorebiastensor = torch.tensor(
            [lrpignorebias], dtype=torch.bool, device=tensorbiasedclass.baseconv.weight.device
        )
        ctx.save_for_backward(
            x,
...
class l2_lrp(torch.autograd.Function):
    ...
    def forward(ctx, conv_features, model, pno):
        ...
        ctx.save_for_backward(
            conv_features, model.prototype_vectors, pno
        )  # *values unpacks the list
...
class L2Conv2dLRPFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, weights):
        # pno = torch.tensor([pno], dtype=torch.int32)
        ctx.save_for_backward(input, weights)  # *values unpacks the list

        ones = torch.ones_like(weights, requires_grad=False)
        ...
        similarities = torch.log((output + 1) / (output + 1e-12))
...
class L2ConvLRP(nn.Module):
    def __init__(self):
        super(L2ConvLRP, self).__init__()

    def forward(self, x, weights):
        return L2Conv2dLRPFunction.apply(x, weights)
